...
const FunnelCTA: React.FC<FunnelCTAProps> = ({
  ...
  const baseStyles = `
    w-full sm:w-auto min-h-[72px] px-8 py-4 text-lg font-semibold rounded-xl 
  ...
  return (
    <button onClick={handleClick} disabled={disabled} className={`${baseStyles} ${variantStyles} ${className}`}>
      <span>{text}</span>
