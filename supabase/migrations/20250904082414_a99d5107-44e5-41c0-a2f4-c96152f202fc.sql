-- Fix PII in security audit logs and improve security functions
...
-- Create a function to clean existing PII from security audit logs
CREATE OR REPLACE FUNCTION public.clean_existing_pii_from_audit_logs()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  -- Hash any existing email addresses in audit logs
  UPDATE public.security_audit_log 
  SET event_data = jsonb_set(
    event_data - 'author_email' - 'email',
    '{author_email_hash}',
    to_jsonb(encode(digest(event_data->>'author_email', 'sha256'), 'hex'))
  )
  WHERE event_data ? 'author_email';
  
  UPDATE public.security_audit_log 
  SET event_data = jsonb_set(
    event_data - 'email',
    '{email_hash}',
    to_jsonb(encode(digest(event_data->>'email', 'sha256'), 'hex'))
  )
  WHERE event_data ? 'email' AND NOT event_data ? 'email_hash';
  ...
  RAISE NOTICE 'PII cleanup completed for security audit logs';
END;
$function$;

-- Run the cleanup function
SELECT public.clean_existing_pii_from_audit_logs();